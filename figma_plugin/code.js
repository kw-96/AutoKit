...
async function initializeState() {
  try {
    // 从clientStorage加载通道ID
    const savedChannelId = await figma.clientStorage.getAsync('figmaMcpChannelId');
    if (savedChannelId) {
      state.channelId = savedChannelId;
      console.log('已加载通道ID:', savedChannelId);
    }
  } catch (error) {
    console.error('加载通道ID失败:', error);
  }
}

// 保存通道ID到clientStorage
async function saveChannelId(channelId) {
  try {
    await figma.clientStorage.setAsync('figmaMcpChannelId', channelId);
    console.log('已保存通道ID:', channelId);
    state.channelId = channelId;
    return true;
  } catch (error) {
    console.error('保存通道ID失败:', error);
    return false;
  }
}
...
figma.ui.onmessage = async (msg) => {
  switch (msg.type) {
    ...
    case "get-channel-id":
      ...
      break;
    case "save-channel-id":
      // 保存通道ID
      const success = await saveChannelId(msg.channelId);
      if (success) {
        figma.notify(msg.channelId ? `通道ID已保存: ${msg.channelId}` : "已清除固定通道ID，将使用随机生成的ID");
      } else {
        figma.notify("保存通道ID失败");
      }
      break;
